...
handle_event(info, Info, State, _Data) ->
    lager:warning("in Data ~p unexpected Info: ~p", [State, Info]),
    keep_state_and_data.
...
handle_capwap_message(Header, {Msg, 0, Seq, Elements},
	...
	{value, {Seq, _, NotifyFun}} ->
	    ...
	    if is_function(NotifyFun) ->
		    Data2 = response_notify(NotifyFun,
					    proplists:get_value(result_code, Elements),
